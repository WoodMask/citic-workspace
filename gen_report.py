...
update_cell(t0, 2, 1, 'AI平台工具域')
update_cell(t0, 3, 1, '智能应用开发平台')
...
t8 = doc.tables[8]
update_cell(t8, 1, 0, 'S智能应用开发平台')
update_cell(t8, 1, 1, 'A')
...
for i, para in enumerate(doc.paragraphs):
    ...
    # Simple text replacements in runs
    for run in para.runs:
        for old, new in replacements.items():
            if old in run.text:
                run.text = run.text.replace(old, new)
    
    # 1.3 引入必要性评估 - main description
    ...
    if '暂无其他Doris数据同步工具' in text:
        for run in para.runs:
            run.text = ''
        para.runs[0].text = '类似大模型包括：GPT-4o（OpenAI）、Claude Opus（Anthropic）、Gemini Pro（Google）、DeepSeek-V3等。GLM-5.1在综合与Coding能力上对齐全球顶尖水平，且在SWE-Bench Pro上表现最优；在长程自主执行、复杂工程优化与真实开发场景中展现出更强的持续工作能力，是中国模型中率先达到8小时级持续工作能力的代表。'
    ...
    if '用于2024-06' in text:
        for run in para.runs:
            run.text = ''
        para.runs[0].text = '用于2026年智能应用开发平台引入大模型能力，支撑智能客服、文档生成、代码辅助等业务场景，后续根据模型版本更新和安全漏洞扫描结果的要求进行升级。'
